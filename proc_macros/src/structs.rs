...
pub(super) fn write_specialization_constants(
    ...
    Ok(quote! {
        ...
        #[allow(unsafe_code)]
        unsafe impl ::rust_vulkan_graphics::all_vulkano::shader::SpecializationConstants for #struct_ident {
            #[inline(always)]
            fn descriptors() -> &'static [::rust_vulkan_graphics::all_vulkano::shader::SpecializationMapEntry] {
                static DESCRIPTORS: [::rust_vulkan_graphics::all_vulkano::shader::SpecializationMapEntry; #num_map_entries] =
                    [ #( #map_entries ),* ];
...
impl ToTokens for TypeFloat {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        if self.width == FloatWidth::W16 {
            tokens.extend(quote! { ::rust_vulkan_graphics::all_vulkano::half:: });
        }
        tokens.append(self.to_ident());
    }
}
...
impl<T: ToTokens> ToTokens for Padded<T> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        if padding == 0 {
            ...
            tokens.extend(quote! { ::rust_vulkan_graphics::all_vulkano::padded::Padded<#ty, #padding> });
        }
    }
}
