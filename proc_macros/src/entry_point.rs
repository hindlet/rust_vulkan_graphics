...
pub(super) fn write_entry_point(
    ...
    quote! {
        (
            #name.to_owned(),
            ::rust_vulkan_graphics::all_vulkano::shader::spirv::ExecutionModel::#model,
            ::rust_vulkan_graphics::all_vulkano::shader::EntryPointInfo {
                execution: #execution,
...
fn write_shader_execution(execution: &ShaderExecution) -> TokenStream {
    match execution {
        ShaderExecution::Vertex => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Vertex },
        ShaderExecution::TessellationControl => {
            quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::TessellationControl }
        }
        ShaderExecution::TessellationEvaluation => {
            quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::TessellationEvaluation }
        }
        ShaderExecution::Geometry(::vulkano::shader::GeometryShaderExecution { input }) => {
            let input = format_ident!("{}", format!("{:?}", input));
            quote! {
                ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Geometry(
                    ::rust_vulkan_graphics::all_vulkano::shader::GeometryShaderExecution {
                        input: ::rust_vulkan_graphics::all_vulkano::shader::GeometryShaderInput::#input
                    }
                )
            }
        }
        ShaderExecution::Fragment(::vulkano::shader::FragmentShaderExecution {
            ...
            quote! {
                ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Fragment(
                    ::rust_vulkan_graphics::all_vulkano::shader::FragmentShaderExecution {
                        fragment_tests_stages: ::rust_vulkan_graphics::all_vulkano::shader::FragmentTestsStages::#fragment_tests_stages,
                    }
                )
            }
        }
        ShaderExecution::Compute => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Compute },
        ShaderExecution::RayGeneration => {
            quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::RayGeneration }
        }
        ShaderExecution::AnyHit => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::AnyHit },
        ShaderExecution::ClosestHit => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::ClosestHit },
        ShaderExecution::Miss => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Miss },
        ShaderExecution::Intersection => {
            quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Intersection }
        }
        ShaderExecution::Callable => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Callable },
        ShaderExecution::Task => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Task },
        ShaderExecution::Mesh => quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::Mesh },
        ShaderExecution::SubpassShading => {
            quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderExecution::SubpassShading }
        }
    }
}
        ...
        descriptor_binding_requirements
            ...
            .map(|(loc, binding_reqs)| {
                ...
                let descriptor_types_items = descriptor_types.iter().map(|ty| {
                    ...
                    quote! { ::rust_vulkan_graphics::all_vulkano::descriptor_set::layout::DescriptorType::#ident }
                });
                ...
                let image_format = match image_format {
                    Some(image_format) => {
                        ...
                        quote! { Some(::rust_vulkan_graphics::all_vulkano::format::Format::#ident) }
                    }
                    None => quote! { None },
                };
                let image_scalar_type = match image_scalar_type {
                    Some(image_scalar_type) => {
                        ...
                        quote! { Some(::rust_vulkan_graphics::all_vulkano::shader::ShaderScalarType::#ident) }
                    }
                    None => quote! { None },
                };
                let image_view_type = match image_view_type {
                    Some(image_view_type) => {
                        ...
                        quote! { Some(::rust_vulkan_graphics::all_vulkano::image::view::ImageViewType::#ident) }
                    }
                    None => quote! { None },
                };
                ...
                let descriptor_items = descriptors.iter().map(|(index, desc_reqs)| {
                    ...
                    let sampler_with_images_items = sampler_with_images.iter().map(|DescriptorIdentifier {
                        ...
                        quote! {
                            ::rust_vulkan_graphics::all_vulkano::shader::DescriptorIdentifier {
                                set: #set,
                                binding: #binding,
                                index: #index,
                            }
                    ...
                    quote! {
                        (
                            #index,
                            ::rust_vulkan_graphics::all_vulkano::shader::DescriptorRequirements {
                ...
                quote! {
                    (
                        (#set_num, #binding_num),
                        ::rust_vulkan_graphics::all_vulkano::shader::DescriptorBindingRequirements {
...
fn write_push_constant_requirements(
    ...
    match push_constant_requirements {
        Some(PushConstantRange {
            ...
            quote! {
                Some(::rust_vulkan_graphics::all_vulkano::pipeline::layout::PushConstantRange {
                    stages: #stages,
                    offset: #offset,
                    size: #size,
                })
        ...
        specialization_constant_requirements
            ...
            .map(|(&constant_id, reqs)| {
                ...
                quote! {
                    (
                        #constant_id,
                        ::rust_vulkan_graphics::all_vulkano::shader::SpecializationConstantRequirements {
                            size: #size,
                        },
                    )
                }
...
fn write_interface(interface: &ShaderInterface) -> TokenStream {
    let items = interface.elements().iter().map(
        |ShaderInterfaceEntry {
            ...
            quote! {
                ::rust_vulkan_graphics::all_vulkano::shader::ShaderInterfaceEntry {
                    location: #location,
                    component: #component,
                    ty: ::rust_vulkan_graphics::all_vulkano::shader::ShaderInterfaceEntryType {
                        base_type: ::rust_vulkan_graphics::all_vulkano::shader::ShaderScalarType::#base_type,
    ...
    quote! {
        ::rust_vulkan_graphics::all_vulkano::shader::ShaderInterface::new_unchecked(vec![
            #( #items ),*
        ])
    }
}
...
fn stages_to_items(stages: ShaderStages) -> TokenStream {
    if stages.is_empty() {
        quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::empty() }
    } else {
        let stages_items = [
            stages.intersects(ShaderStages::VERTEX).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::VERTEX }
            }),
            stages
                .intersects(ShaderStages::TESSELLATION_CONTROL)
                .then(|| {
                    quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::TESSELLATION_CONTROL }
                }),
            stages
                .intersects(ShaderStages::TESSELLATION_EVALUATION)
                .then(|| {
                    quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::TESSELLATION_EVALUATION }
                }),
            stages.intersects(ShaderStages::GEOMETRY).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::GEOMETRY }
            }),
            stages.intersects(ShaderStages::FRAGMENT).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::FRAGMENT }
            }),
            stages.intersects(ShaderStages::COMPUTE).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::COMPUTE }
            }),
            stages.intersects(ShaderStages::RAYGEN).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::RAYGEN }
            }),
            stages.intersects(ShaderStages::ANY_HIT).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::ANY_HIT }
            }),
            stages.intersects(ShaderStages::CLOSEST_HIT).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::CLOSEST_HIT }
            }),
            stages.intersects(ShaderStages::MISS).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::MISS }
            }),
            stages.intersects(ShaderStages::INTERSECTION).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::INTERSECTION }
            }),
            stages.intersects(ShaderStages::CALLABLE).then(|| {
                quote! { ::rust_vulkan_graphics::all_vulkano::shader::ShaderStages::CALLABLE }
            }),
        ]
        .into_iter()
        .flatten();
